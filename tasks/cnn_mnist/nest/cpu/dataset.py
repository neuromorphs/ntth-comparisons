    ...
    def __init__(self, dataset, time_stimulus):
        ...
        for cell in range(cells):
            ...
            for sample in range(samples):
                tmp = np.concatenate((tmp, self.train_set[sample].spike_AER[cell] + ((time_stimulus['duration'] + time_stimulus['silence']) * np.array(sample))))
        ...
        for cell in range(cells):
            tmp = np.array([])
            for sample in range(samples):
                tmp = np.concatenate((tmp, self.test_set[sample].spike_AER[cell] + ((time_stimulus['duration'] + time_stimulus['silence']) * np.array(sample))))
    ...
    def __init__(self, sample, time_stimulus):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                if rate == 0:
                    self.spike_AER.append(np.array([]))  # no stimulus
                else:
                    spike_sequence = []
                    ...
                    while spike_time < time_stimulus['duration']:
                        ...
                        poisson_isi = -math.log(1.0 - random.random()) / rate * 1000.0  # ms tau
                        spike_time += poisson_isi
                    self.spike_AER.append(np.array(spike_sequence))
